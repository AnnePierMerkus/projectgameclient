...
public class GameAI extends GameOptions {
	...
	public void updateFromGame() {
		// Update the board
		for(TileUI tileUI : this.gameOptions.getBoard().getGameBoard().values()) {
			this.board.getTile(tileUI.getIndex()).setOccupant(tileUI.getOccupant());
		}
...
//		}
		...
		for(Player p : PlayerList.players.values()) {
			for(TileUI tileUI : this.gameOptions.getBoard().getFilledTiles().get(p.getId()).values()) {
				this.board.addFilledTile(p, tileUI);
			}
		}
	...
	public void makePredictionMove(int tileIndex, Player player) {
		...
		this.getGameProperty().makeMove(this.getBoard().getTile(tileIndex), player);
	...
	public void makePredictionMove(int tileIndex, Player player, Board board) {
		//this.board.getTile(tileIndex).setOccupant(player);
		this.getGameProperty().makeMove(board.getTile(tileIndex), player);
		//this.getGame().makeMove(this.board.getTile(tileIndex), player);
	}
