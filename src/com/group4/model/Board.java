...
public class Board {
	...
	public void savePrevious(int threadId, int moveCount, Tile tile, Player player) {
		if(!this.previousBoard.containsKey(threadId)) this.previousBoard.put(threadId, new HashMap<Integer, HashMap<Integer, Tile>>());
		if(!this.previousBoard.get(threadId).containsKey(moveCount)) this.previousBoard.get(threadId).put(moveCount, new HashMap<Integer, Tile>());
	...
	public void savePrevious(int threadId, Tile tile, Player player) {
		if(!this.previousBoard.containsKey(threadId)) this.previousBoard.put(threadId, new HashMap<Integer, HashMap<Integer, Tile>>());
		if(!this.previousBoard.get(threadId).containsKey(this.moveCounter.get(threadId))) this.previousBoard.get(threadId).put(this.moveCounter.get(threadId), new HashMap<Integer, Tile>());
		Tile prevTile = new Tile(tile.getIndex(), tile.getWeight());
		prevTile.setOccupant(player, threadId);
		this.previousBoard.get(threadId).get(this.moveCounter.get(threadId)).put(tile.getIndex(), prevTile);
	}
	...
	public void revert(int threadId, int moves) {
		...
		for(int counter = 0; counter < moves; counter++) {
			...
			for(Tile tile : this.previousBoard.get(threadId).get(this.moveCounter.get(threadId)).values()) {
				if(tile.isOccupied()) {
	...
	public void resetFilledTiles(int threadId) {
		if(!this.filledTiles.containsKey(threadId)) this.filledTiles.put(threadId, new HashMap<String, HashMap<Integer, Tile>>());
	...
	public void addFilledTile(int threadId, Player player, Tile tile) {
		...
		if(!this.filledTiles.containsKey(threadId)) this.filledTiles.put(threadId, new HashMap<String, HashMap<Integer, Tile>>());
		if(!this.filledTiles.get(threadId).containsKey(player.getId())) this.filledTiles.get(threadId).put(player.getId(), new HashMap<Integer, Tile>());
		if(!this.filledTiles.get(threadId).containsKey(otherPlayerId)) this.filledTiles.get(threadId).put(player.getId(), new HashMap<Integer, Tile>());
